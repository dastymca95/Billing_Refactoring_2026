...
| Frontend boots but the API returns CORS errors | The browser is hitting `http://backend:8000` instead of going through the Vite proxy. | Make sure you set `VITE_API_BASE_URL=http://backend:8000` (only valid inside the docker network) and you load the page from `http://localhost:5173` (not `http://backend:5173`). |
