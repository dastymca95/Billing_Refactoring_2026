...
function openCase(x){current=x.benchmark_id;caseTitle.textContent=x.benchmark_id;meta.textContent=JSON.stringify({cohort:x.cohort,page_count:x.page_count,quality:x.quality_metrics,warnings:x.inventory_warnings,duplicates:x.duplicate_information,preview_rotation_degrees:x.preview_rotation_degrees,filename_folder_candidates:x.filename_folder_candidates},null,2);preview.innerHTML=`<iframe src="${x.preview_url}?token=${TOKEN}" style="transform:rotate(${x.preview_rotation_degrees||0}deg);transform-origin:center"></iframe>`;replacement.innerHTML='<option value="">automatic same-cohort reserve replacement</option>'+x.reserve_candidates.map(r=>`<option value="${r.benchmark_id}">${r.benchmark_id} · ${r.quality_tier}</option>`).join('');label.value=JSON.stringify(template(),null,2);refresh()}
